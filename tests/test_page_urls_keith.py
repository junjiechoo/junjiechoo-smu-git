...
def test_uploadmaterials_page(client, session, captured_templates):
    ...
    assert context['trainer'] == 'T003'
    # assert list(context['courses']).count() >= 1
    # assert list(context['lessons']).count() >= 1
    # assert list(context['materials']).count() >= 1



#/learner/enrolment need to do
